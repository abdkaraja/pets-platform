...
- **Global scope**: `C:/Projects/pet-platform/.opencode/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `C:/Projects/pet-platform/.opencode/notes/`)
...
2. Glob `C:/Projects/pet-platform/.opencode/notes/*.md` (if directory exists) — global notes
...
Global (C:/Projects/pet-platform/.opencode/notes/):
...
2. **No `.planning/`**: Falls back to global `C:/Projects/pet-platform/.opencode/notes/` — works in any directory
